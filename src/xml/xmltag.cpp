...
rapidxml::xml_node<>* CXmlPathBuilder::get_node_from_path(const xmlpath& path)
{
  rapidxml::xml_node<>* current_node = xml_doc.get();
...
void CXmlPathBuilder::build_path_under_node(rapidxml::xml_node<>* base,
  const xmlpath& path)
{
  ...
  for (auto iter = path.begin(); iter != path.end(); iter++)
  {
    if ((!already_built) || (get_child(current_node, iter -> c_str()) == nullptr))
    {
      already_built = 0;
      add_child(base, iter -> c_str());
    }
  }
}
